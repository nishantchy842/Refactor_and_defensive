...
const processTransaction = (transaction) => {
  const transactions = {
    ...
    debit: (amount) => console.log("debit card", amount),
    transfer: (amount) => console.log("transfer", amount),
  };

  transaction.forEach((item) => {
    const transactionFun = transactions[item.type];

    if (item.amount > 0) {
      if (!transactionFun) throw new Error("Invalid type");
      return transactionFun(item.amount);
    }
  });
};
